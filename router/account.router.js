...
router.get('/:email/:password/:role', (req, res) => {
    Accountreg.find({ email: req.params.email, password: req.params.password, role: req.params.role }, (err, result) => {
        if (err)
            res.status(404).send(err)
        else
            res.status(200).send(result);
...
router.put('/:id', (req, res) => {
    Accountreg.findByIdAndUpdate({ _id: req.params.id }, {
        $Set: {
            username: req.body.username,
            password: req.body.password,
            email: req.body.email,
