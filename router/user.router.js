...
router.get('/:id/:username', (req, res)=>{
    user.findById({_id:req.params.username},(err, result)=>{
        if(err)
        res.status(404).send(err)
        else
        res.status(200).send(result);
...
router.post('/', async(req,res)=>{
    ...
    //create new User
    let newUser=new user(_.pick(req.body,['username','password', 'role']))
    const salt=await bcrypt.genSalt(10);
    newUser.password=await bcrypt.hash(newUser.password, salt)
    await newUser.save();
    const token=jwt.sign({_id:newUser._id, username:newUser.username},config.get('jwtPrivateKey'));
    res.header('x-auth-token', token).status(200).send(_.pick(newUser,['_id','username']))
})
